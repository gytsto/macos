  ...
  if [[ "${NETWORK,,}" == "bridge"* ]]; then
    for ((i = 0; i < ETH_COUNT; i++)); do
      ...
      for j in $(seq 1 $RETRIES); do
        OUTPUT=$(python3 /run/qga.py --sh "ifconfig en$i| grep status | awk '{print \$2}'")
        STATUS=$(echo "$OUTPUT" | grep -A1 'STDOUT:' | tail -n1 | tr -d '\r' | xargs)

        echo "Status: '$STATUS'"
        if [[ "$STATUS" == "active" ]]; then
          echo "Interface 'en$i' is up!"
          break
        else
          echo "Waiting for interface 'en$i' to be up... ($j/$RETRIES)"
          sleep 1
        fi
      done

      exit_code=0
      python3 /run/qga.py --sh "networksetup -setmanual "$INTERFACE_NAME" '$CURRENT_IP' 255.255.255.0 '$GW'" || exit_code=$?
      if [[ $exit_code -ne 0 ]]; then
        echo "Failed to set ip address using qga.py" >&2
        return 3
      fi

      python3 /run/qga.py --sh "networksetup -setdnsservers '$INTERFACE_NAME' 1.1.1.1" || exit_code=$?
      if [[ $exit_code -ne 0 ]]; then
...
NET_OPTS+=" -device virtio-serial-pci,id=virtserial0,bus=pcie.0,addr=0x7"
